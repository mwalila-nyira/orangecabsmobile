...
router.get("/trippaid", function(req,res,next){
...
router.post("/trippaid", function(req,res,next){
    var booking = req.body;
    var mobile = booking.mobile;
    var token = booking.token;

     //Insert into the data base
     connection.query('SELECT * FROM trips WHERE mobile=? AND access_token=? AND status_pay="paid"  AND is_delete="0"  ORDER BY trip_id DESC',
     [mobile,token],function(error,results,fields){
        ...
        if(results.length > 0){
            ...
            res.send({'message':'You have not taken a trip yet.'});
        }
